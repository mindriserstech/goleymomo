...
class SalesController extends Controller
{
    ...
    function salesEntry(Request $request){
        ...
        if($request->file('sales_voucher')) {
            ...
            $query = $sale->save();
            $check_userid = User::where('id', Auth::id())->select('user_id')->first();
            ...
            if(!empty($sales_grand) && ($sales_grand->is_paid == "not_paid")){
                ...
                if(!is_null($check_userid->user_id) && (!empty($sales_partner_grand)) && ($sales_partner_grand->is_paid == "not_paid")){
                    $p_total_amt = $request->card_amount + $request->cash_amount + $sales_partner_grand->total_amt;
                    $p_commission_payable = (($this->get_commission_rate($user_id)/100) * ($request->card_amount + $request->cash_amount)) + $sales_partner_grand->commission_payable;
                    $partner_data = [
                    ...
                    $partner_data = [
                        ...
                        'entry_date' => date('Y-m-d'),
                        'is_paid' => 'not_paid',
                        'commission_payable' => ($this->get_commission_rate($user_id)/100) *  ($request->card_amount + $request->cash_amount),
                        'due_amt' => ($this->get_commission_rate($user_id)/100) *  ($request->card_amount + $request->cash_amount),
                    ];
                ...
                if(!is_null($check_userid->user_id) && (!empty($sales_partner_grand)) && ($sales_partner_grand->is_paid == "not_paid")){
                    ...
                    $total_amt = $request->card_amount + $request->cash_amount + $sales_partner_grand->total_amt;
                    $commission_payable = (($this->get_commission_rate($user_id)/100) * ($request->card_amount + $request->cash_amount)) + $sales_partner_grand->commission_payable;
                    $data = [
                        'total_amt' => $total_amt,
                        'user_id' => $user_id,
                        'entry_date' => date('Y-m-d'),
                    ...
                    DB::table('sales_partner_grand')->where('user_id', $user_id)->where('is_paid', 'not_paid')->update($data);
                }else{
                    DB::table('sales_partner_grand')->where('user_id', $user_id)->insert($data);
                }
            }
